...
def validate_project_name(value: str) -> str:
    ...
    if not re.match(r'^[a-zA-Z0-9][a-zA-Z0-9_-]*$', value):
        raise click.BadParameter(
            'Project name should start with a letter or number and contain only letters, numbers, underscores, and hyphens, but not start with underscores or hyphens.'
        )
...
def create_file(file_path: Union[Path, str], content: Optional[str] = None) -> None:
...
def create_project() -> None:
    ...
    need_tests: str = click.prompt("Create tests(pytest) directory? (Y/n)", default='Y', type=str, show_default=False)

    # Copy template files
    template_path: Path = BASE_PATH / 'template'
    shutil.copy(template_path / 'pyproject.toml', project_path / 'pyproject.toml')
    shutil.copy(template_path / '.gitignore', project_path / '.gitignore')
    shutil.copy(template_path / '.pre-commit-config.yaml', project_path / '.pre-commit-config.yaml')
    ...
    with open(project_path / 'pyproject.toml', 'r') as _file:
